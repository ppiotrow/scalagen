...
import utils.SampleActors.TestControllerActor
...
with StopSystemAfterAll {
  "A Controller actor" must {
    " update a population" in {
      ...
      val expectedCouples = (phenotypes(0), phenotypes(1)) ::(phenotypes(0), phenotypes(2)) :: Nil
      expectMsg(UpdatePopulation(expectedCouples, expectedToBeKilled))
    }
  }

  "A calculation of population changes" must {
    "return max possible kills when population is too big" in {
      ...
      val (toKill, toCreate) =
        Controller.calculatePopulationChange(currentPopulationSize, optimalPopulationSize, maxKillOrCreate)
